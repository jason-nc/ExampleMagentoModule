...
class Mage_Customer_Model_Address_Abstract extends Mage_Core_Model_Abstract
{
	...
	$a = true;
	if(stripos('po box',$this->getStreet(1)) !== false){
		$a = false;
	}else if(stripos('p.o. box',$this->getStreet(1)) !== false){
		$a = false;
	}else if(stripos('po. box',$this->getStreet(1)) !== false){
		$a = false;
	}else if(stripos('p.o box',$this->getStreet(1)) !== false){
		$a = false;
	}else if(stripos('p.o.box',$this->getStreet(1)) !== false){
        ...
        if (!Zend_Validate::is($this->getStreet(1), 'NotEmpty')) {
            ...
            $this->addError(Mage::helper('customer')->__('Cannot use a P.O. box for an address.'));
        }
