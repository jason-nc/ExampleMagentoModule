...
class Mage_Sales_Model_Order_Item extends Mage_Core_Model_Abstract
{
    ...
    {
	        Mage::log('in getStatus call',null,'tsmSalesModelOrderItem.log');
    ...
    public function getItemAltitude() {
	        Mage::log('in getItemAltidude call',null,'tsmSalesModelOrderItem.log');
	 	$itemId = $this->getId();
	        ...
	        Mage::log('query: ' . $query,null,'tsmSalesModelOrderItem.log');
	...
	/*      $query = "SELECT q.* FROM `sales_order_entity_int` o
	        ...
	        while ($row = $res->fetch() ) {
	            if(key_exists('altitude',$row)) {
	                echo nl2br($row['altitude']);
	            }
	        }
	        Mage::log('done getItemAltidude call',null,'tsmSalesModelOrderItem.log');
	    }    
    	public function getItemAltitudeByQuoteId($quoteId) {
	 	...
	 	$itemId = $this->getId();
	        ...
	        WHERE o.item_id = ' . $itemId;
	        Mage::log('query: ' . $query,null,'tsmSalesModelOrderItem.log');
	 	$query = 'SELECT * FROM sales_flat_quote_item WHERE quote_id = ' . $quoteId . ' AND quote_item_id = ' . $itemId;
	        # For older versions of Magento
	/*      $query = "SELECT q.* FROM `sales_order_entity_int` o
	        LEFT JOIN `sales_flat_quote_item` q on o.value = q.entity_id
	        WHERE o.entity_id = $itemId AND o.attribute_id = 343";       */    
