...
//        }
    ...
    public function removeErrorInfosByParams($params)
    {
    	...
    	public function getItemAltitudeByQuoteId($quoteId) {
	        Mage::log('in getItemAltitudeByQuoteId',null,'tsmSalesModelQuoteItem.log');
	 	$itemId = $this->getId();
	        $write = Mage::getSingleton('core/resource')->getConnection('core_write');
	 
	 	$query = 'SELECT * FROM sales_flat_quote_item WHERE quote_id = ' . $quoteId . ' AND item_id = ' . $itemId;
	        Mage::log('query: ' . $query,null,'tsmSalesModelQuoteItem.log'); 
	 
	        $res = $write->query($query);
	 	$altitude = '';
	        while ($row = $res->fetch() ) {
	            if(key_exists('altitude',$row)) {
	                $altitude = nl2br($row['altitude']);
	            }
	        }
	        Mage::log('done getItemAltitudeByQuoteId',null,'tsmSalesModelQuoteItem.log');
	        return $altitude;
	    }    
	    /*
    	public function setItemAltitudeByQuoteId($quoteId, $altitude) {
	        Mage::log('in getItemAltitudeByQuoteId - quoteID: ' . $quoteId . ' - altitude: ' . $altitude,null,'tsmSalesModelQuoteItem.log');
	 	$itemId = $this->getId();
	        Mage::log('Item Id: ' . $itemId,null,'tsmSalesModelQuoteItem.log');
	        $write = Mage::getSingleton('core/resource')->getConnection('core_write');
	 
	 	$query = 'UPDATE sales_flat_quote_item SET altitude = "' . $altitude . '" where item_id = ' . $itemId . ' AND quote_id = ' . $quoteId;
	        Mage::log('query: ' . $query,null,'tsmSalesModelQuoteItem.log'); 
