...
class Mage_Checkout_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
	        ...
	        Mage::log('Before itemInfo[altitude] check',null,'tsmCheckoutModelCart.log');
            /* Start: Custom code added for comments */
	    if(!empty($itemInfo['altitude'])) {
	        Mage::log('Altitude is not empty: ' . $itemInfo['altitude'],null,'tsmCheckoutModelCart.log');
		...
		$quote_id = $session->getQuoteId();
	        ...
	        $query = 'UPDATE sales_flat_quote_item SET altitude = "' .$itemInfo['altitude'] . '" WHERE item_id = ' . $item->getId() . ' AND quote_id = ' . $quote_id;
	        $write->query($query);
	        Mage::log('Cart query: ' . $query,null,'tsmCheckoutModelCart.log');
	 
	        $item->setItemcomment($itemInfo['altitude']);
	    }
